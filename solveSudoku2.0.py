...
class Grid:
    ...
    def __init__(self, rows, cols, width, height, win):
        self.rows = rows
        self.cols = cols
        self.squares = [[Square(self.matrix[i][j], i, j, width, height) for j in range(cols)] for i in range(rows)]
        self.width = width
        self.height = height
        self.model = None
        self.update_model()
...
def redraw_window(win, matrix, time, strikes):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_RETURN:
                    i, j = matrix.selected
                    if matrix.squares[i][j].temp != 0:
                        if matrix.place(matrix.squares[i][j].temp):
                            print("Success")
                        else:
                            print("Wrong")
                            strikes += 1
                        ...
                        if matrix.is_finished():
                            print("Game over")

            if event.type == pygame.MOUSEBUTTONDOWN:
                pos = pygame.mouse.get_pos()
                clicked = matrix.click(pos)
                if clicked:
                    matrix.select(clicked[0], clicked[1])
                    key = None

        if matrix.selected and key != None:
            matrix.sketch(key)

        redraw_window(win, matrix, play_time, strikes)
        pygame.display.update()
